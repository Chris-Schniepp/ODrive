...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if pos > -(full_length-4000) and joystick.get_axis('x') > .15:
                if switch:
                    print("right")
                    switch = False
                axis.set_vel(-22000*joystick.get_axis('x'))
                # axis.set_pos(pos-(1000*joystick.get_axis('x')))
            elif pos < -4000 and joystick.get_axis('x') < -.15:
                if not switch:
                    print("left")
                    switch = True
                axis.set_vel(22000*-joystick.get_axis('x'))
